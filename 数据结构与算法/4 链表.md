...
链接方式存储的[[2 线性表|线性表]]简称为链表。
- 用一组任意的存储单元存储表
- 每个数据元素(ai)不但包含元素值(data)，还必须存储相邻数据元素(ai-1)或(ai+1)的地址。
## 单链表
每个节点包含两个域，data存储该节点的元素，next存储下一节点的地址
![[Pasted image 20260104143142.png]]

分为有表头和无表头
![[Pasted image 20260104143223.png]]
### 操作
#### 插入
![[Pasted image 20260104143307.png]]
```c
...
时间复杂度：$O(1)$
#### 删除
![[Pasted image 20260104143526.png]]
```c
...
空表的时候：`list==null`
![[Pasted image 20260104144428.png]]
#### 头插法
...
表头结点位于表的最前端，本身不带数据
![[Pasted image 20260104145137.png]]
...
![[Pasted image 20260104202815.png]]
...
单个节点的结构：
![[Pasted image 20260104202949.png]]

循环双链表结构：
![[Pasted image 20260104203000.png]]

- 判断空表的条件：
	- `head == head->pri == head->next`
	- ![[Pasted image 20260104203051.png]]
...
插入前：
![[Pasted image 20260104203448.png]]
插入后：
![[Pasted image 20260104203526.png]]
#### 删除
将节点p删除：
```c
...
free(p);                    //3.释放节点p的空间
```
![[Pasted image 20260104204125.png]]
![[Pasted image 20260104204134.png]]
## 约瑟夫斯问题
...
![[Pasted image 20260104214255.png]]
运行过程中，X和Y通过键盘输入字符构建单链表，合并后打印双链表M的数据。

> 链表的定义：
> ![[Pasted image 20260104214322.png]]
> 主函数：
> ![[Pasted image 20260104214500.png]]
> 节点插入函数：
> ![[Pasted image 20260104214548.png]]
